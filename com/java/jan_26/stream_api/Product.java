...
public class Product {
    ...
    public static void main(String[] args) {
        ...
        productList.stream().filter(p->p.getId()==1).forEach(System.out::println);
        //null safe
        Optional<Product> product = productList.stream().filter(p -> p.getId() == 3).findFirst();
        System.out.println(product);

        System.out.println("=====all id's only for all products ===");
        productList.stream().map(Product::getId).forEach(System.out::println);

        System.out.println("=====id's >2 for all products ===");
        ...
        System.out.println("=========all sorted price==========");
        productList.stream().map(Product::getPrice).sorted().forEach(System.out::println);

        System.out.println("=========all price only==========");
        productList.stream().map(Product::getPrice).forEach(System.out::println);
        ...
        Product product1 = productList.stream().sorted(Comparator.comparing(Product::getPrice).reversed()).skip(0).findFirst().orElse(null);
        ...
        Product lowestPrice = productList.stream().sorted(Comparator.comparing(Product::getPrice)).skip(0).findFirst().orElse(null);
        System.out.println(lowestPrice);

        System.out.println("======3rd lowest price product=====");
        Product thirdLowestPrice = productList.stream().sorted(Comparator.comparing(Product::getPrice)).skip(3).findFirst().orElse(null);
        System.out.println(thirdLowestPrice);
    }
